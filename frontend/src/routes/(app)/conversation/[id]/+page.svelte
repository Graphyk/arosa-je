<script lang="ts">
  ...
  // Pas compris la partie avec le provider tu m expliqueras demain
  // const dataProvider = getContext<DataProvider>("dataProvider");

  // Aussi il faut gérer pour faire en sorte que les derniers messages soient en premier

</script>

<div class="mx-auto w-full bg-white h-screen flex flex-col">
    <div class="flex p-2 py-4 bg-[#F0F4FA]  w-full">
      <img src={conversation.contact.profile_picture} class="h-16 w-16 ml-4 mr-8 bg-gray-300 rounded-full" alt="Profil du contact"/>
  ...
  <div class="flex-1 bg-gray-50 flex flex-col min-h-0">
      ...
      <ul class="grid h-auto gap-4 mt-4 mx-2 overflow-y-scroll">
        {#each conversation.messages as message}
          ...
          {:else}
            <li class="w-2/3 ml-auto mr-2 p-2 bg-green-700 text-white border-2 rounded-lg">{message.content}</li>
          {/if}
        {/each}
      </ul>
      <!-- Adapt the bottom to move the textarea -->
      <div class="flex w-full mt-auto gap-2">
        <textarea class="w-full  mt-auto border-green-700 rounded-md" bind:value={textValue}>
        </textarea>
